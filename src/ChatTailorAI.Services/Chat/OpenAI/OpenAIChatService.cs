...
namespace ChatTailorAI.Services.Chat.OpenAI
{
    public class OpenAIChatService : IOpenAIChatService
    {
        ...
        private List<RootObject> RetrieveSelectedFunctions()
        {
            ...
            string filePath = Path.Combine(AppContext.BaseDirectory, "Configuration/function-schemas.json");

            string jsonSchemas = File.ReadAllText(filePath);
            
            List<RootObject> functionSchemas = JsonService.SafeDeserialize<List<RootObject>>(jsonSchemas);
        ...
        private async Task<OpenAIChatResponseDto> ProcessNonStreamResponse(OpenAIChatRequest chatRequest, HttpResponseMessage response)
        {
            if (!response.IsSuccessStatusCode)
            {
                ...
                var errorResponse = JsonService.SafeDeserialize<OpenAIChatCompletionErrorResponse>(errorContent);
            ...
            var chatCompletion = JsonService.SafeDeserialize<OpenAIChatCompletionResponse>(responseContent);
            ...
            while (!streamReader.EndOfStream)
            {
                ...
                if (line.StartsWith("data: "))
                {
                    ...
                    OpenAIChatCompletionResponse chatCompletion = 
                        JsonService.SafeDeserialize<OpenAIChatCompletionResponse>(data);

                    if (chatCompletion == null || chatCompletion.Choices == null)
                    {
                        var error = JsonService.SafeDeserialize<OpenAIChatCompletionErrorResponse>(data);
        ...
        {
            string jsonBody = JsonService.SafeSerialize(body);
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                var errorResponse = JsonService.SafeDeserialize<OpenAIChatCompletionErrorResponse>(data);
