...
namespace ChatTailorAI.Shared.ViewModels.Pages
{
    public class ChatPageViewModel : Observable, IDisposable
    {
        ...
        private async Task RecordAudio()
        {
            ...
            {
                _inAppNotificationService.Display(ex.Message);
            }
        }
        ...
        private async void OnChatMessageReceived(object sender, MessageReceivedEvent message)
        {
            ...
            // If the message is not for the current conversation, ignore it
            if (message.ChatId != CurrentConversation.Id) return;
        ...
        private async Task CheckIfNewChat(string message = null)
        {
            ...
            {
                _inAppNotificationService.Display("No conversation set");
                return;
            }
            ...
            try
            {
                ...
                if (CurrentConversation.AssistantType == AssistantType.OpenAI)
                {
                    ...
                    if (assistant == null)
                    {
                        ...
                        _inAppNotificationService.Display("Unable to message assistant, it may have been deleted. Please create a new chat.");
        ...
        private async Task SendChatRequest(string message, List<string> imageUris = null)
        {
            ...
            try
            {
                ...
                var chatRequest = chatRequestBuilder.BuildChatRequest(model, modelInstructions, apiMessages, chatSettings, CurrentConversation.Id);
                ...
                if (botChatResponseDto.Content != null)
                {
                    ...
                    _appNotificationService.ShowNotification("New Message", botChatMessageDto.Content);
        ...
        {
            _inAppNotificationService.Display("Feature temporarily disabled until a future release");
            return;
        ...
        public void DeleteMessage(ChatMessageViewModel message)
        {
            ...
            if (IsAssistantChat)
            {
                _inAppNotificationService.Display("Cannot delete messages in an OpenAI assistant chat");
                return;
            }
        ...
        private async Task AttachImage()
        {
            ...
            _inAppNotificationService.Display($"Attached {imagesToAttach.Count} images");
        }
        ...
        {
            if (CurrentConversation != null && CurrentConversation.Id == e.Conversation.Id)
            {
        ...
        public async Task EditChat()
        {
            ...
            {
                _inAppNotificationService.Display("Unable to update, OpenAI Assistants don't support vision models");
                return;
            }
            ...
            _inAppNotificationService.Display("Chat updated successfully");
        }
    }
}